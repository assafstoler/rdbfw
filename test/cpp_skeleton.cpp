...
static void *skel_event(void *p) {
    ...
    while (break_requested == 0) {
        ...
        do {
            ...
            q = (rdbmsg_queue_t *) rdb_delete(ctx->msg_q_pool, 0, NULL);
...
static void skel_event_pre_init(void *p) {
    ...
    if ( route_skel < 0 || group_skel < 0 ) {
        fwl ( LOG_FATAL, ctx, "Unable to register route, or group message: %d:%d",
...
static void skel_event_init(void *p) {
    ...
    fwl (LOG_INFO, NULL, "Initilizing %s\n", ctx->name);
    ...
    if (0 != rdbmsg_request(p, route_timers, route_skel, rdbmsg_lookup_id ("GROUP_TIMERS"), 
        ...
        fwl (LOG_ERROR, ctx, "rdbmsg_request failed. events may not fire. Aborting");
...
extern "C" const rdbfw_plugin_api_t cpp_skeleton_rdbfw_fns = {
